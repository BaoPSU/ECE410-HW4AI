...
The module has three stages. First stage is the weight register, it's a flip-flop that latches the full weight matrix when you pulse weight_load high. Claude packed the entire 4-by-4 matrix into a single 16-bit flat register, where bit 4i plus j holds weight[i][j]. For an example, bit zero is weight[0][0], bit four would be weight[1][0], and so on. I asked Claude why it went with a flat register and it said it actually loads the entire matrix in exactly one clock cycle. Like if you loaded row by row, that would've taken four cycles and you'd need an address counter to track which row you're on, so the flat register just keeps it simple.
