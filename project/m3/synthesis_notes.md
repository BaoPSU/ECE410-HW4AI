...
**Synthesis result.** OpenLane v2.3.10 (dockerized) synthesized the integrated `top` design into 7,671 sky130_fd_sc_hd cells, 84,848 µm² total area (≈ 0.085 mm²). That's a **45% area reduction** vs CF07's unpipelined 17,029 cells / 0.155 mm², even though we *added* 693 flip-flops worth of pipeline registers. The reason is that splitting the combinational cone at the kdist→argmin boundary plus the mid-argmin boundary lets yosys optimize each stage independently with smaller gate variants, and DIST_W=20→18 shaves two bits off every accumulator and comparator on the path. 0 inferred latches, 0 lint errors. Full numbers in `synth/area_report.txt`.
...
**Post-synth checker bypassed.** Even after the V-2005 conversion, OpenLane's `08-checker-yosyssynthchecks` step flagged 2 "Drivers conflicting with a constant" warnings from yosys's internal address-decode inference and treated them as fatal. The synthesis itself completes cleanly (yosys final-check pass reports 0 problems on the optimized netlist), so the warnings are about specific bits of yosys-internal registers being statically determinable, not about design correctness. **Workaround:** invoke OpenLane with `--to Yosys.Synthesis`, which runs only up to and including the synthesis stage, skipping the failing post-checker and the downstream PnR + STA + power stages. This is enough to extract cell count + area for M3; M4 will either refactor the const-driver pattern away (register the AXI address before decode) or set `LINT_WARNINGS_AS_ERRORS: false` to demote the failure and let the full flow produce a real STA + power report.
